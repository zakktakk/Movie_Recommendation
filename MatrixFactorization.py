...
class MatrixFactorization:
    ...
    def __init__(self, rating, dimension, steps=30000, gamma=0.005, lmd=0.02, threshold=0.001):
        self.rating = rating
        self.mu = np.mean(rating)
        self.b_u = np.mean(self.rating, axis=1, keepdims=True) - self.mu
        self.b_i = np.mean(self.rating, axis=0) - self.mu
    ...
    def get_userfactor(self, rating):
        evaluated_num = np.asarray([np.count_nonzero(user) for user in rating])
